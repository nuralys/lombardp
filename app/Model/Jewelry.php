...
class Jewelry extends AppModel{
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/jewelry/' . $fileName;
		$path_th = WWW_ROOT . 'img/jewelry/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 164, 225, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/jewelry/' . $name)){
